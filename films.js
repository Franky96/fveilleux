  ...
  onSnapshot(filmsDocRef, (docSnap) => {
    if (docSnap.exists()) {
      ...
      if (filmsData.items.length === 0) {
        ...
        setDoc(filmsDocRef, filmsData);
      } else if (filmsData.dataVersion < 3) {
        ...
        filmsData.items = filmsData.items.map(i => ({
          ...i,
          trailer_youtube: (i.trailer_youtube || "") === "" && trailersManquants[i.titre]
            ? trailersManquants[i.titre]
            : (i.trailer_youtube || ""),
        }));
        filmsData.dataVersion = 3;
        setDoc(filmsDocRef, filmsData);
      }
    } else {
      filmsData = donneesDemo;
